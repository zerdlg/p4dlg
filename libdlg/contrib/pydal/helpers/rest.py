            ...
            Use as::
                ...
                def index():
                    def GET(*args,**sqVars):
                        ...
                        parser = db.parse_as_rest(patterns,args,sqVars)
                        if parser.status == 200:
                            return dict(content=parser.response)
                        else:
                            raise HTTP(parser.status,parser.error)

                    def POST(table_name,**sqVars):
                        if table_name == 'person':
                            return db.person.validate_and_insert(**sqVars)
                        elif table_name == 'pet':
                            return db.pet.validate_and_insert(**sqVars)
                        else:
                            raise HTTP(400)
                    return locals()
        """
