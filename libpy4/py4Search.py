...
'''  [$File: //dev/p4dlg/libpy4/py4Search.py $] [$Change: 452 $] [$Revision: #8 $]
...
class Search(P4QControl):
    ...
    def __call__(
        ...
        ''' search results
        '''
        results_raw = self.getresults(self.parser(sTerms))
        results = self.as_dict(results_raw)
        return results
    ...
    def get_words(self, item):
        return self.parse(item) \
            ...
            else reduce(self.mergeitems, [self.get_words(i) for i in item]) \
    ...
    def subwords(self, word, d=3):
        ...
        for i in range(0, (n - d + 1)):
            for j in range((i + d), (n + 1)):
                if (j > i):
                    yield word[i:j]

    def bin(self, words, subs=True):
        (hist, parents) = (Storage(), Storage())
    ...
    def parseresults(
        ...
        for key in parents:
            for number in index.get(key, []):
                ...
                acc[number] = (
                        (acc[number] or 0.0) + (count1 * count2)
                )
        results = Lst(Lst([relevance, number]) for (number, relevance) in iter(acc.items()))  # acc.iteritems())
    ...
    def printer(self, results):
        ...
        vecTable.align = 'l'
        [vecTable.add_row(result.getvalues()) for result in results if (len(results) > 0)]
        print(vecTable)
